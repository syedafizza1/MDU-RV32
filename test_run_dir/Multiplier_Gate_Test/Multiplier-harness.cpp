...
#endif
#include <iostream>
class Multiplier_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    Multiplier_api_t(VMultiplier* _dut) {
        dut = _dut;
        main_time = 0L;
        is_exit = false;
#if VM_TRACE
    ...
    void init_sim_data() {
        ...
        sim_data.inputs.push_back(new VerilatorIData(&(dut->io_src_b)));
        sim_data.inputs.push_back(new VerilatorIData(&(dut->io_src_a)));
        sim_data.outputs.push_back(new VerilatorIData(&(dut->io_output_bits)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_output_valid)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_ready)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["Multiplier.reset"] = 0;
    }
#if VM_TRACE
...
static Multiplier_api_t * _Top_api;
...
int main(int argc, char **argv, char **env) {
    ...
    VMultiplier* top = new VMultiplier;
    std::string vcdfile = "test_run_dir/Multiplier_Gate_Test/Multiplier.vcd";
    ...
    Verilated::mkdir("test_run_dir/Multiplier_Gate_Test/logs");
    VerilatedCov::write("test_run_dir/Multiplier_Gate_Test/logs/coverage.dat");
#endif
    delete top;
    exit(0);
}
